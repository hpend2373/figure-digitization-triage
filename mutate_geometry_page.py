...
MUT = [
    ...
    ("M14 값의 출처를 늘 읽은 값이라고 적는다",
     "      source = picked ? 'TYPED_PICKED' : 'TYPED';", "      source = 'READ';"),
    ...
    ("M28 찍은 줄이 있어도 잰 눈금에 붙인다",
     "      var rowTop = picked ? Number(s.pickedTopPixel) : s.topPixel;\n      var rowBottom = picked ? Number(s.pickedBottomPixel) : s.bottomPixel;",
     "      var rowTop = s.topPixel;\n      var rowBottom = s.bottomPixel;"),
    ("M29 프레임 밖에 찍은 줄도 받는다",
     "      if (picked && !rowsInsideFrame(rowTop, rowBottom, s.frameTop, s.frameBottom)) {",
     "      if (false) {"),
    ("M30 프레임 선 위의 눈금을 밖이라고 한다",
     "  var slack = Math.abs(bt - t) * FRAME_SLACK;", "  var slack = 0;"),
    ("M31 찍은 값의 출처를 친 값과 구별하지 않는다",
     "      source = picked ? 'TYPED_PICKED' : 'TYPED';", "      source = 'TYPED';"),
    ("M32 한 줄만 찍어도 찍은 것으로 친다",
