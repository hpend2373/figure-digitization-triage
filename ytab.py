...
def main(path="trY.csv"):
    ...
    print("  panels with a local ladder %d; panels in a group with a provider %d"
          % (own, prov + len(dep)))
    ...
    for key in sorted({(r["pid"], r["fig"]) for r in groups}):
        ...
        print("  %-5s %-10s groups %d  %s" % (
            ...
            ", ".join("%s[%s prov=%s n=%s]" % (x["group_id"],
                                               x["status"].replace("Y_SCALE_GROUP_", ""),
                                               x["provider_panel"] or "-",
