...
# ------------------------------------------ 작은 조각 수백 개가 그림 하나다
# 선 그래프는 눈금·선분 하나하나가 별개 객체이고 어느 것도 혼자서는 그림이
# 아닙니다. run2에서 691개 중 690개가 "작다"고 버려져 캡션에 답할 후보가
# 없었습니다. 크기는 뭉친 뒤에 잽니다.
_segments = [(60 + i * 3.0, 200 + (i % 7) * 4.0, 63 + i * 3.0, 203 + (i % 7) * 4.0)
             for i in range(90)]                         # 3pt짜리 조각 90개
_ticks = [(60 + i * 20.0, 196.0, 60.6 + i * 20.0, 199.0) for i in range(14)]
_tiny_chart = FR.cluster(_segments + _ticks, PAGE, [])
check("작은 조각 수백 개로 그려진 그래프도 후보 하나가 된다",
      len(_tiny_chart) == 1, _tiny_chart)
check("그 후보가 조각 전체를 덮는다",
      _tiny_chart and _tiny_chart[0][0] <= 60 and _tiny_chart[0][2] >= 329, _tiny_chart)
check("하지만 혼자 떨어진 작은 조각은 여전히 후보가 아니다",
      FR.cluster([(300, 300, 303, 303)], PAGE, []) == [])
