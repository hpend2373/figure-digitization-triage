"""The geometry of a monochrome bar panel, and which series each bar is.

Imported by BOTH the diagnostic driver (`measure_mono_bars.py`) and, once the
integration lands, the production reader - because two implementations of one
measurement drift, and this project has already paid for that: the prototype
read publication 397's WOMEN panel for weeks while the production reader was
never pointed at it, and the two disagreed about that panel's stroke, its caps
and its footprints without anything noticing.

Nothing here opens a figure or decides what to do with an answer. It measures,
and it refuses: every function that cannot establish what it was asked for says
so by name rather than returning a number it cannot defend. `measure_mono_bars`
holds the spec format, the corpus and the printing; `mark_readers` will hold the
manifest contract. A caller that wants the geometry should not have to import
the diagnostic corpus to get it.

Every length here is a multiple of the figure's own measured stroke, every
intensity feature is reported at four thresholds and once without one, and a
...
def _gray_from_rgb(rgb):
    """Greyscale, using cv2's weighting where it is available."""
    if cv2 is not None:
...
def geometry_rows(gray, panel_box, calibration, anchors, fills,
    ...
    reader that names a series while it measures a panel is naming it from the
    only thing a panel knows - where the bar sits - which is the inference this
    package refuses. `fills` is the DECLARATION to check a measurement against,
    never an identification; `fill_identities_by_figure` names the series once
    every panel of the figure has been measured.

    Takes an already-loaded greyscale array and any calibration object with
    `value_to_pixel` and `pixel_to_value`, so nothing here has to know how a
    figure is stored or which module owns the axis maths - which is also what
    keeps this module out of an import cycle with the production reader.
