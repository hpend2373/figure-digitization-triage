...
```
CORE 3441   FULL 3482   RASTER_ONLY 286     래스터 포함: 3727 / 3768     28 suites
...
- (v9.19에서 durable group evidence 2-grain과 `pilot_beckers` schema drift는 닫혔습니다.)
- 코퍼스가 있어야 하는 것: negative gold case(475 Fig.1 `PANEL_TOPOLOGY_REVIEW_REQUIRED`,
  177 Fig.2 `Y_SCALE_GROUP_NO_PROVIDER`), `pilot_323.py` + CI 배선, geometry_proposer를
  plan 층에 연결, 전체 코퍼스 재측정. 지금 컨테이너에는 코퍼스가 없습니다.
