...
MUT = [
 ...
 ("M4 행 존재 검사 제거", "if (!row) { rejected.push({ id: id, reason: 'ROW_GONE' }); continue; }",
                          "if (!row) { row = rows[0]; }"),
 ("M5 크롭 차단 제거", "  if (row.Count_Blocked === '1') {", "  if (false) {"),
 ...
 ("M10 막힌 행의 이의를 버림",
  "    if (disputed) return 'BLOCK_DISPUTED';", "    if (false) return 'x';"),
 ("M16 차단 확인을 버림",
  "    if ((confirmed || {})[row.Draft_ID]) return 'BLOCK_CONFIRMED';",
  "    if (false) return 'BLOCK_CONFIRMED';"),
 ("M17 확인이 이의를 이김",
  "    if (disputed) return 'BLOCK_DISPUTED';",
  "    if (disputed && !(confirmed || {})[row.Draft_ID]) return 'BLOCK_DISPUTED';"),
 ...
 ("M18 확인 표를 검사하지 않음", "  if (s !== CONFIRM_MARK) {", "  if (false) {"),
]
